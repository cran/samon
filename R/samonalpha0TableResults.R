alpha0TableResults <- function( Y1, samonR1, Y2, samonR2, samonRD ) {
  ...
  optP1  <- matrix(samonR1$HM,nrow=1)
  optQ1  <- matrix(samonR1$FM,nrow=1)
  optP2  <- matrix(samonR2$HM,nrow=1)
  optQ2  <- matrix(samonR2$FM,nrow=1)
...
##  IFRes[3,]    <- c(TMD0[ 18 ], CID0[ "lb7" ], CID0[ "ub7" ], CID0[ "lb8" ], CID0[ "ub8" ])
  ...
  IFRes[1,]    <- c(TM10[      "IFEst" ], CI10[ "lb7" ], CI10[ "ub7" ], CI10[ "lb8" ], CI10[ "ub8" ])
  IFRes[2,]    <- c(TM20[      "IFEst" ], CI20[ "lb7" ], CI20[ "ub7" ], CI20[ "lb8" ], CI20[ "ub8" ])
  IFRes[3,]    <- c(TMD0[ "Difference" ], CID0[ "lb7" ], CID0[ "ub7" ], CID0[ "lb8" ], CID0[ "ub8" ])
  colnames(IFRes) <- c("Estimate", "LCLBootstrap", "UCLBootstrap", "LCLJK", "UCLJK" ) 

  CIlevel <- samonRD$CIlevel
  if ( is.null(CIlevel) ) CIlevel <- .95

  # table 7 Optimal sigmaH and sigmaF  
  optRes <- rbind( cbind( 1, optP1, optQ1 ), cbind( 2, optP2, optQ2 ) )
